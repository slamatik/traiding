...
# todo get more data,
#  add table with intraday values
#  look for ways to automate data collection on daily basis (1min, 5mmin, 15mmin, 1hr, 4hr, 1d)
...
def get_prices(period='10y'):
    ...
    for i in range(0, len(names), interval):
        ...
        df = yf.download(ticker_slice, group_by='ticker', period=period)
        for idx in range(len(names_slice)):
            temp_df = df[ticker_slice[idx]].dropna()

            temp_df['ticker'] = ticker_slice[idx]
            temp_df['ticker_id'] = idx_slice[idx]
            temp_df.to_sql('daily', conn, if_exists='append')
            # else DataFrame.to_sql('stock_price', conn if_exists='append')
